import { BoardConfig, BoardRow, PlaybackMode, SoundClip, SoundPad } from '../types';
...
// Version 4 Mannschaften, aber kein Wiedergabemodus und keine Lautstaerke je Sound.
...
/** Fehlende Felder aelterer Versionen mit Standardwerten auffuellen. */
function normalizePad(pad: Partial<SoundPad>): SoundPad {
  const teams = Array.isArray(pad.teams) ? pad.teams.filter(t => ALL_TEAM_IDS.includes(t)) : [];
  const clips = (pad.clips ?? []).map(c => ({ ...c, gain: typeof c.gain === 'number' && c.gain >= 0 && c.gain <= 1 ? c.gain : 1 }));
  return {
    id: pad.id ?? newId(),
...
/** Akzeptiert v1 bis v5, liefert immer ein v5-Board oder null. */
export function migrateBoard(raw: unknown): BoardConfig | null {
  if (!raw || typeof raw !== 'object') return null;
  const b = raw as { version?: number };
  if ((b.version === 5 || b.version === 4 || b.version === 3) && Array.isArray((b as LegacyRows).rows)) {
    const rows = (b as LegacyRows).rows.map(r => ({ id: r.id ?? newId(), pads: (r.pads ?? []).map(normalizePad) }));
    return { version: 5, rows };
  }
